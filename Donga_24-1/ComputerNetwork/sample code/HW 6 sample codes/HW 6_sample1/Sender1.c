...
int check_timeout(ListMark *tq)
{
    ...
    // 0보다 크면 0.5초가 지났다는 의미ㄴ
}

int makePackets()
{
    FILE *file = fopen("text.txt", "rb"); // "rb" 모드로 열기
    if (!file)
    {
...
int main()
{
    ...
    isAcked = (int *)malloc(packet_count * sizeof(int)); // ack된 것을 확인하기 위한 배열
    ...
    while (send_base < pkt_count)
    {
        ...
        Add_Last_Sender(list, p); // 내부적으로 타임아웃 세팅 들어가있음
        next_pkt++;
        usleep(50000);
    }
