...
int findIndex(char* buf) {
	for (int i = 0; i < PACKET_COUNT; i++) 
		if (strcmp(packet_msg[i], buf) == 0) return i;
...
int recvData() {
	...
	int index = findIndex(buf);
	received_packets[index] = strdup(buf); // 수신한 패킷 저장
...
int sendData() {
	...
	int recv_packet = findIndex(buf);

	strcpy(buf, ack_msg[recv_packet]);
	expect_ack++;
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		// 받은 패킷 처리
		if (strcmp(buf, packet_msg[expect_ack]) == 0) {
		
			if (deliver == 0) printf("\"%s\" is received. ", buf);
			else {
				printf("\"%s\" is received. ", buf);

				for (int i = retrans_packet; i < PACKET_COUNT; i++) {
						if (i == PACKET_COUNT - 1) printf("and ");
						printf("%s", received_packets[i]);
		...
		} else if (strcmp(buf, packet_msg[expect_ack]) != 0) {
			printf("\"%s\" is received and buffered. ", buf);

			retrans_packet = expect_ack--;
			deliver = 1; retrans_msg = 1;
