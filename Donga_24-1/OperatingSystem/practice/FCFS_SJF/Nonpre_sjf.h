...
void enqueue(Queue* q, Process* newProcess) {                
    ...
    Process* compareProcess = q->front->next;   // 비교할 프로세스 
    Process* prevProcess = q->front;            // 이전 프로세스

    while (1) {
        // 두번째로 들어온 프로세스 처리, 현재 큐의 프로세스보다 실행시간이 긴 프로세스 처리
        ...
        // 작업시간 비교하여 큐에 저장
        if (newProcess->burstTime < compareProcess->burstTime) {
            prevProcess->next = newProcess;
            newProcess->next = compareProcess;
            return;
        } 

        // 그 다음 비교할 프로세스와 이전 프로세스 설정
