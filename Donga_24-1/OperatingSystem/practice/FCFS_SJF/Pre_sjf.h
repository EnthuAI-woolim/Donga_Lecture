    ...
    while (1) {
        ...
        if (compareProcess->interupt == 1) {
            while (1) { // 작업시간이 커서 선점 안할 경우 적절한 위치 찾기위함
                ...
                // 1 : 선점하는 프로세스의 실행시간, 0 : 다른 프로세스의 실행시간
                if (matchInterupt == 1) 
                    compareBurstTime = compareProcess->burstTime - (newProcess->arrivalTime - compareProcess->arrivalTime);
                else
                    compareBurstTime = compareProcess->burstTime;

                
                // 작업시간 비교하여 큐에 저장
                if (matchInterupt == 1 && newProcess->burstTime < compareBurstTime) {
                    // 밀린 프로세스 생성, 설정
                    ...
                    while (1) {
                        ...
                        if (overdueProcess->burstTime < compareProcess->burstTime) {
                ...
                if (matchInterupt == 0 && newProcess->burstTime < compareBurstTime) {
